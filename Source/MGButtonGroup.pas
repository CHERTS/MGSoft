...
{ #                                                                          # }
{ #  MGButtonGroup v1.0.0 - Группа кнопок                                    # }
{ #                                                                          # }
{ #  License: GPLv3                                                          # }
{ #                                                                          # }
...
type
  ...
  /// <summary>
  /// Класс для создания собственных цветов в MGButtonGroups
  /// </summary>
  ...
  /// <summary>
  /// Основной класс MGButtonGroup
  /// </summary>
  TMGButtonGroup = class(TButtonGroup)
  private
...
{$REGION 'Doc'}
      /// <summary>
      /// ButtonGradient
      /// Если ButtonGradient = True, то кнопка рисуется градиентом От и До
      /// </summary>
{$ENDREGION}
...
procedure TMGButtonGroup.ColorsChange(Sender: TObject);
begin
  Invalidate;
end;

constructor TMGButtonGroup.Create(AOwner: TComponent);
begin
...
destructor TMGButtonGroup.Destroy;
begin
  FColors.Free;
  inherited Destroy;
end;

procedure TMGButtonGroup.DrawButton(Index: Integer; Canvas: TCanvas; Rect: TRect;
...
procedure TMGButtonGroup.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  ix: Integer;
begin
  inherited;
...
procedure TMGButtonGroup.SetButtonGradient(const Value: Boolean);
begin
...
procedure TMGButtonGroup.SetButtonGradientDirection(const Value: TGradientDirection);
begin
...
procedure TMGButtonGroup.SetDrawFocusRect(const Value: Boolean);
begin
...
procedure TMGButtonGroup.SetButtonBorder(const Value: Boolean);
begin
