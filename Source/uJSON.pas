...
const
  CROTINA_NAO_IMPLEMENTADA :string = 'Feature not implemented';
...
begin
 {$IFDEF MSWINDOWS}
  ...
  //SysUtils.GetLocaleFormatSettings (LOCALE_SYSTEM_DEFAULT, F);
 {$ELSE}
    newNotImplmentedFeature();
 {$ENDIF}
 {$IFDEF DELPHI16_UP}
   FS := TFormatSettings.Create(Windows.GetThreadLocale);
   FS.DecimalSeparator := '.';
   FS.ThousandSeparator := ',';
   FS.CurrencyDecimals := 2;
   Result := FS;
   Result.DecimalSeparator := FS.DecimalSeparator;
   Result.ThousandSeparator := FS.ThousandSeparator;
   Result.CurrencyDecimals := FS.CurrencyDecimals;
 {$ELSE}
   Result := F;
